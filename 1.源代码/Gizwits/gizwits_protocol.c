...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagwaterPump)
    {
        bitFieldBitLen += waterPump_LEN;
    }
    if(0x01 == aFlag->flagfan)
    {
        bitFieldBitLen += fan_LEN;
    }
    if(0x01 == aFlag->flagvalve)
    {
        bitFieldBitLen += valve_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagwaterPump)
    {
        info->event[info->num] = EVENT_waterPump;
        info->num++;
        dataPoints->valuewaterPump = gizVarlenDecompressionValue(bitFieldOffset,waterPump_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += waterPump_LEN;
    }
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagvalve)
    {
        info->event[info->num] = EVENT_valve;
        info->num++;
        dataPoints->valuevalve = gizVarlenDecompressionValue(bitFieldOffset,valve_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += valve_LEN;
    }
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagflameValue)
    {
        info->event[info->num] = EVENT_flameValue;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueflameValue,(uint8_t *)&issuedData[byteFieldOffset],flameValue_LEN);
        byteFieldOffset += flameValue_LEN;
        dataPoints->valueflameValue = gizX2Y(flameValue_RATIO,  flameValue_ADDITION, elongateP0FormTmp.devStatus.valueflameValue); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagsmogValue)
    {
        info->event[info->num] = EVENT_smogValue;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuesmogValue,(uint8_t *)&issuedData[byteFieldOffset],smogValue_LEN);
        byteFieldOffset += smogValue_LEN;
        dataPoints->valuesmogValue = gizX2Y(smogValue_RATIO,  smogValue_ADDITION, exchangeBytes(elongateP0FormTmp.devStatus.valuesmogValue));
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagmethaneValue)
    {
        info->event[info->num] = EVENT_methaneValue;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuemethaneValue,(uint8_t *)&issuedData[byteFieldOffset],methaneValue_LEN);
        byteFieldOffset += methaneValue_LEN;
        dataPoints->valuemethaneValue = gizX2Y(methaneValue_RATIO,  methaneValue_ADDITION, exchangeBytes(elongateP0FormTmp.devStatus.valuemethaneValue));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuewaterPump != cur->valuewaterPump)
    {
        GIZWITS_LOG("valuewaterPump Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagwaterPump = 1;
        ret = 1;
    }
    if(last->valuesmogAlarm != cur->valuesmogAlarm)
    {
        GIZWITS_LOG("valuesmogAlarm Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagsmogAlarm = 1;
        ret = 1;
    }
    if(last->valuemethaneAlarm != cur->valuemethaneAlarm)
    {
        GIZWITS_LOG("valuemethaneAlarm Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagmethaneAlarm = 1;
        ret = 1;
    }
    if(last->valueflameAlarm != cur->valueflameAlarm)
    {
        GIZWITS_LOG("valueflameAlarm Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagflameAlarm = 1;
        ret = 1;
    }
    ...
    if(last->valuepeople != cur->valuepeople)
    {
        GIZWITS_LOG("valuepeople Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagpeople = 1;
        ret = 1;
    }
    if(last->valueflame != cur->valueflame)
    {
        GIZWITS_LOG("valueflame Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagflame = 1;
        ret = 1;
    }
    if(last->valuevalve != cur->valuevalve)
    {
        GIZWITS_LOG("valuevalve Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagvalve = 1;
        ret = 1;
    }
    ...
    if(last->valueflameValue != cur->valueflameValue)
    {
        GIZWITS_LOG("valueflameValue Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagflameValue = 1;
        ret = 1;
    }
    if(last->valuesmogValue != cur->valuesmogValue)
    {
        GIZWITS_LOG("valuesmogValue Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagsmogValue = 1;
        ret = 1;
    }
    if(last->valuemethaneValue != cur->valuemethaneValue)
    {
        GIZWITS_LOG("valuemethaneValue Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagmethaneValue = 1;
        ret = 1;
    }
    ...
    if(last->valuemethane != cur->valuemethane)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuemethane Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagmethane = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagwaterPump)
    {
        gizVarlenCompressValue(bitFieldOffset,waterPump_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuewaterPump);
        bitFieldOffset += waterPump_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagsmogAlarm)
    {
        gizVarlenCompressValue(bitFieldOffset,smogAlarm_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuesmogAlarm);
        bitFieldOffset += smogAlarm_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagmethaneAlarm)
    {
        gizVarlenCompressValue(bitFieldOffset,methaneAlarm_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuemethaneAlarm);
        bitFieldOffset += methaneAlarm_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagflameAlarm)
    {
        gizVarlenCompressValue(bitFieldOffset,flameAlarm_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueflameAlarm);
        bitFieldOffset += flameAlarm_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagpeople)
    {
        if(dataPoints->valuepeople >= people_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valuepeople Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,people_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuepeople);
                bitFieldOffset += people_LEN;
        }
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagflame)
    {
        if(dataPoints->valueflame >= flame_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valueflame Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,flame_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueflame);
                bitFieldOffset += flame_LEN;
        }
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagvalve)
    {
        if(dataPoints->valuevalve >= valve_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valuevalve Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,valve_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuevalve);
                bitFieldOffset += valve_LEN;
        }
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagflameValue)
    {
        devStatusTmp.valueflameValue = gizY2X(flameValue_RATIO,  flameValue_ADDITION, dataPoints->valueflameValue); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueflameValue,flameValue_LEN);
        byteFieldOffset += flameValue_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagmethane)
    {
        devStatusTmp.valuemethane = exchangeBytes(gizY2X(methane_RATIO,  methane_ADDITION, dataPoints->valuemethane)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuemethane,methane_LEN);
        byteFieldOffset += methane_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagsmogValue)
    {
        devStatusTmp.valuesmogValue = exchangeBytes(gizY2X(smogValue_RATIO,  smogValue_ADDITION, dataPoints->valuesmogValue)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuesmogValue,smogValue_LEN);
        byteFieldOffset += smogValue_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagmethaneValue)
    {
        devStatusTmp.valuemethaneValue = exchangeBytes(gizY2X(methaneValue_RATIO,  methaneValue_ADDITION, dataPoints->valuemethaneValue)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuemethaneValue,methaneValue_LEN);
        byteFieldOffset += methaneValue_LEN;
    }
